...
namespace EPayments.Data.Repositories.Implementations
{
    public class EquationControlsRepository : IEquationControlsRepository
    {
        ...
        public async Task<List<BoricaTransactionVO>> GetBoricaTransactions(DateTime? dateFrom, 
            ...
            int resultsPerPage)
        {
            var predicate = this.CreateBoricaTransaction(dateFrom, dateTo, transactionStatus);

            return await this.SortBoricaTransactionRequests(
            ...
            bool sortDescending)
        {
            ...
            return await this.SortBoricaTransactionRequests(this.UnitOfWork.DbContext.Set<BoricaTransaction>()
                .Where(predicate), sortBy, sortDescending)
                .Select(BoricaTransactionVO.MapFrom)
                .ToListAsync();
        }
        ...
        public async Task<int> CountBoricaTransactions(DateTime? dateFrom, 
            DateTime? dateTo, 
            int? transactionStatus)
        {
            var predicate = this.CreateBoricaTransaction(dateFrom, dateTo, transactionStatus);

            return await this.UnitOfWork.DbContext.Set<BoricaTransaction>()
                .Where(predicate)
                .CountAsync();
        }
        ...
        public async Task<decimal> GetTransactionsAmounts(DateTime? dateFrom,
            DateTime? dateTo,
            int? transactionStatus)
        {
            var predicate = this.CreateBoricaTransaction(dateFrom, dateTo, transactionStatus);

            decimal? amount = await this.UnitOfWork.DbContext.Set<BoricaTransaction>()
                .Where(predicate)
                .SumAsync(tr => (decimal?)tr.Amount);

            return amount.HasValue ? amount.Value : 0;
        }

        public async Task<decimal> GetTransactionsFees(DateTime? dateFrom,
            DateTime? dateTo,
            int? transactionStatus)
        {
            var predicate = this.CreateBoricaTransaction(dateFrom, dateTo, transactionStatus);

            decimal? fee = await this.UnitOfWork.DbContext.Set<BoricaTransaction>()
                .Where(predicate)
                .SumAsync(tr => tr.Fee);

            return fee.HasValue ? fee.Value : 0;
        }

        public async Task<decimal> GetTransactionsCommissions(DateTime? dateFrom,
            DateTime? dateTo,
            int? transactionStatus)
        {
            var predicate = this.CreateBoricaTransaction(dateFrom, dateTo, transactionStatus);

            decimal? commission = await this.UnitOfWork.DbContext.Set<BoricaTransaction>()
                .Where(predicate)
                .SumAsync(tr => tr.Commission);

            return commission.HasValue ? commission.Value : 0;
        }
        ...
        private Expression<Func<BoricaTransaction, bool>> CreateBoricaTransaction(DateTime? dateFrom,
            DateTime? dateTo,
            int? transactionStatus)
        {
            ...
            {
                predicate = predicate.And(e => e.TransactionDate >= dateFrom.Value);
            }

            if (dateTo.HasValue)
            {
                predicate = predicate.And(e => e.TransactionDate < dateTo.Value);
            }
