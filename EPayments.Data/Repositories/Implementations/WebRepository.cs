    ...
    internal class WebRepository : BaseRepository, IWebRepository
    {
            ...
            int resultsPerPage)
        {
            IQueryable<PendingRequestVO> query =
                from pr in this.unitOfWork.DbContext.Set<PaymentRequest>().Include(i => i.InitiatorEserviceClient)
                join ac in this.unitOfWork.DbContext.Set<EserviceClient>().Include(ac => ac.ObligationType) on pr.EserviceClientId equals ac.EserviceClientId
            ...
            int resultsPerPage)
        {
            ...
            IQueryable<ProcessedRequestVO> query =
                ...
                .Include(pr => pr.EserviceClient.ObligationType)
                .Where(predicate)
                .Select(pr =>
                    new ProcessedRequestVO()
                    {
                        ...
                        ObligationType = pr.EserviceClient.ObligationType.Name,
                        InitiatorName = pr.InitiatorEserviceClient.ServiceName
                    });
        ...
        public IList<PendingRequestVO> GetPendingRequestsByAccessCode(string accessCode)
        {
            return
                (from pr in this.unitOfWork.DbContext.Set<PaymentRequest>()
                 join ac in this.unitOfWork.DbContext.Set<EserviceClient>().Include(ec => ec.ObligationType) on pr.EserviceClientId equals ac.EserviceClientId
                 join vpc in this.unitOfWork.DbContext.Set<VposClient>() on ac.VposClientId equals vpc.VposClientId into g1
                 from vpc in g1.DefaultIfEmpty()
        ...
        {
            return
                (from pr in this.unitOfWork.DbContext.Set<PaymentRequest>().Include(pr => pr.EserviceClient.ObligationType)
            ...
            PaymentRequestStatus? filterRequestStatus)
        {
            ...
            predicate = predicate.And(e =>
                e.ApplicantUin == uin &&
                e.PaymentRequestStatusId != PaymentRequestStatus.Pending &&
            ...
            if (String.IsNullOrWhiteSpace(accessCode))
            {
                if (uin == null)
                {
                    query =
                        ...
                        on cl.ObligationTypeId equals ot.ObligationTypeId
