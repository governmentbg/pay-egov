...
namespace EPayments.Job.Host.Jobs.Distributions
{
    public class DistributionJob : IJob
    {
        ...
        public void Action(CancellationToken token)
        {
            ...
            {
                JobLogger.Get(JobName.DistributionJob).Log(LogLevel.Error, $"{ex.Message}, Inner -> {ex.InnerException} , Inner2 -> {ex.InnerException.InnerException}");
            }
            catch (OperationCanceledException ex)
            {
                JobLogger.Get(JobName.EserviceNotification)
                    .Log(LogLevel.Error, "Distribution job was canceled due to a token cancellation request", ex);

                JobLogger.Get(JobName.DistributionJob).Log(LogLevel.Error, ex.Message);
            }
            catch (Exception ex)
            {
                JobLogger.Get(JobName.EserviceNotification).Log(LogLevel.Error, $"{ex.Message}, StackTrace -> {ex.StackTrace}");
        ...
        {
            JobLogger.Get(JobName.EserviceNotification).Log(LogLevel.Info, $"Start Distribute -> token:{token.IsCancellationRequested}");
            ...
            lock (syncRoot)
            {
                ...
                ICVPosRegisterManager cvposRegisterManager = tuple.Item3;
                ISystemRepository systemRepository = tuple.Item4;
                IJobRepository jobRepository = tuple.Item5;
                ...
                while (true)
                {
                    ...
                    JobLogger.Get(JobName.EserviceNotification).Log(LogLevel.Info, $"Start Distribute -> distributionRevenues:{distributionRevenues}");
                    ...
                    {
                        JobLogger.Get(JobName.EserviceNotification).Log(LogLevel.Info, $"Distribute -> foreach ->");
                        ...
                        BnbFile bnbFile = distributionFactory.BnbModelCreator()
                           .Create(distributionRevenue, bulstat, eGov, iban, bicCode, vpn, vd, firstDescription, secondDescription);
                        ...
                        //    AppSettings.EPaymentsCommon_UseMachineKeySet ?
                        //    X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.PersistKeySet | X509KeyStorageFlags.Exportable :
                        //    X509KeyStorageFlags.PersistKeySet | X509KeyStorageFlags.Exportable))
                        //{
                        //    using (RSA rsa = certificate.GetRSAPrivateKey())
                        //    {
                        //        document = bnbXmlDocumentCreator.SignDocument(document, rsa);
                        //    }
                        //}
                        ...
                        this.UpdateDistribution(distributionRevenue, DateTime.Now.ToUniversalTime(), fileName);
        ...
        private DistributionRevenue UpdateDistribution(DistributionRevenue distributionRevenue, DateTime dateTime, string fileName)
        {
            distributionRevenue.DistributedDate = dateTime;
