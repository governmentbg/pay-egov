            ...
            using (var factory = this.dependencyFactory())
            {
                ...
                while (startDate < DateTime.Now)
                {
                    try
                    {
                        ...
                        JobLogger.Get(JobName.EserviceNotification).Log(LogLevel.Info, $"ProcessPendingCVposRequest -> cVposTransaction_Agency:{cVposTransaction_Agency}, " +
                        ...
                        else
                        {
                            ...
                            {
                                string transactionOrder = String.Format(
                                    "{0}{1}",
                                    Formatter.DateToShortFormat(recEventTransaction.transactionDate),
                                    recEventTransaction.orderId.Replace(prefix, string.Empty).TrimStart(new char[] { '0' }));

                                JobLogger.Get(JobName.EserviceNotification).Log(LogLevel.Info, $"ProcessPendingCVposRequest -> transactionOrder:{transactionOrder}");

                                BoricaTransaction currentBoricaTransaction = unitOfWork.DbContext.Set<BoricaTransaction>()
                                    .Include(bt => bt.PaymentRequests)
                                    .FirstOrDefault(bt => bt.Order == transactionOrder && bt.TransactionStatusId == (int)BoricaTransactionStatusEnum.Paid);
