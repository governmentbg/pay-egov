    ...
    public partial class VposController : BaseController
    {
        ...
        public string EpayPaymentResult(string encoded, string checksum)
        {
            if (checksum == EpayVposHelper.CalculateHmac(AppSettings.EPaymentsWeb_EpayVposSecret, encoded))
            {
                ...
                EpayResultData resultData = EpayVposHelper.GetEpayResultData(vposResultData);
        ...
        public virtual ActionResult Borica(Guid[] gids)
        {
            ...
            List<PaymentRequest> paymentRequests = this.webRepository.GetPendingPaymentRequestByUid(gids);

            if (paymentRequests.Count != gids.Length)
            {
            ...
            if (apgwResponse.IsResponseValid())
            {
                ...
                using (var rsa = certificate.GetRSAPublicKey())
                {
                    ...
                    if (isValid)
                    {
                        ...
                        {
                            paymentRequest.PaymentRequestStatusId = PaymentRequestStatus.InProcess;
                        }
                        unitOfWork.Save();
                                ...
                                case TrTypeEnum.Payment:
                                    ...
                                    if (apgwResponse.Rc == "00")
                                    {
                                        ...
                                        foreach (var pr in boricaTransaction.PaymentRequests)
                                        {
                                            ...
                                            {
                                                EserviceNotification statusNotification = new EserviceNotification(pr, 1);
                                    ...
                                    break;
                                    //case TrTypeEnum.Authorization:
                                    //    if (string.Equals(apgwResponse.Rc, MasterCardSoftRc, StringComparison.OrdinalIgnoreCase) || string.Equals(apgwResponse.Rc, VisaCardSoftRc, StringComparison.OrdinalIgnoreCase))
                                    //    {
                                    //        return this.SoftBoricaAuthorization(apgwResponse);
                                    //    }

                                    //    return this.CreateEndingAuthorization(apgwResponse);
                                    //case TrTypeEnum.Reversal:
                                    //case TrTypeEnum.AuthorizationReversal:
                                    //    boricaTransaction = apgwResponse.UpdateBoricaTransactionCanceledPayment(boricaTransaction);

                                    //    foreach (PaymentRequest paymentRequest in boricaTransaction.PaymentRequests)
                                    //    {
                                    //        if (paymentRequest.ObligationStatusId == ObligationStatusEnum.IrrevocableOrder)
                                    //        {
                                    //            paymentRequest.PaymentRequestStatusId = PaymentRequestStatus.Authorized;
                                    ...
                                    //    TempData[TempDataKeys.IsVposPaymentSuccessful] = true;
                                    //    TempData[TempDataKeys.VposPaymentMessage] = "Плащането беше отменено.";
            ...
            bool createVposResult)
        {
            ...
            if (resultStatus == EserviceVposResultStatus.Success)
            {
                ...
                if (!String.IsNullOrWhiteSpace(paymentRequest.AdministrativeServiceNotificationURL))
                {
                    if (hasVposRedirect)
                    {
                        EserviceNotification statusNotification = new EserviceNotification(paymentRequest, 1);
